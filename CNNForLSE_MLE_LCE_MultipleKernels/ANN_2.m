...
TP_testing = double (testingSize) - 0.25;
...
kernel = (0.01)*rand(5,5,2);% four 5 X 5 convolutional kernels
...
thetas = (0.01)*rand(2*fix(size(F,1)/2)*fix(size(F,2)/2),14);%final layer thetas, have 14 subject classes
...
Accuracy =  (double (TP_training)/double (trainingSize));
disp('Testing Accuracy: ');
disp(Accuracy);

Accuracy =  (double (TP_testing)/double (testingSize));
