...
data{2,1} = Y1;
rng(2,'philox');
kernel = (0.1)*rand(40,40); % could be multiplied by 0.01 to get interesting results
initial = kernel;
F = zeros(size(data{1,1},1)-size(kernel,1)+1,size(data{1,1},2)-size(kernel,2)+1);
thetas = (0.1)*rand(fix(size(F,1)/1)*fix(size(F,2)/1),1);
trainingSize = 2;
iterations = [];
avgs = [];
iteration = 0;
...
while iteration <=200
    ...
    dJ = (((H{1,1}'))*(processValues-Y)) + (alpha)*(tmpThetas);
...
title('cross-entropy loss vs Iteration Number');
